...
static void print_window(base_t *coordinates, char **array, nb_t *number,
    int nb)
{
    if (LINES <= number->lines || COLS <= number->cols) {
        clear();
        if (my_strlen("Please, grow the terminal") < COLS) {
            mvprintw(LINES / 2, (COLS - my_strlen("Please, grow the terminal"))
            / 2, "Please, grow the terminal");
        } else {
                mvprintw(LINES / 2, (COLS - my_strlen("Please, grow term"
                )) / 2, "Please, grow term");
        }
        refresh();
    } else {
